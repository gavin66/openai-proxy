...
func Proxy(c *gin.Context) {
	...
	aUrl := Config.Endpoint + c.Request.URL.Path
	// POST request with all data and headers
	var req *http.Request
	var err error
	if c.Request.Method == "POST" {
		req, err = http.NewRequest("POST", aUrl, c.Request.Body)
		if err != nil {
		...
		req, err = http.NewRequest("PATCH", aUrl, c.Request.Body)
		if err != nil {
			c.JSON(500, gin.H{"message": "Internal server error"})
			return
		}
	} else if c.Request.Method == "DELETE" {
		req, err = http.NewRequest("DELETE", aUrl, c.Request.Body)
		if err != nil {
			c.JSON(500, gin.H{"message": "Internal server error"})
			return
		}
	...
	if Config.Proxy != "" {
		...
		client = &http.Client{Transport: transport, Timeout: time.Second * 360}
	}
